...
export default function DoctorDashboard() {
  ...
  async function loadPatientNotes() {
    ...
    try {
      ...
      const url = predictionId.trim()
        ? `/doctors/predictions/${encodeURIComponent(
            predictionId.trim()
          )}/notes`
  ...
  async function addNote() {
    ...
    try {
      const res = await fetch("/doctors/notes", {
        ...
        throw new Error(
          "Network error: Failed to reach /doctors/notes (CORS/proxy/redirect?)"
        );
      });
      ...
      if (!res.ok) {
        const msg =
          payload?.error ||
          (payload?.html
            ? "Auth redirect / non-JSON response from server"
            : `HTTP ${res.status}`);
        throw new Error(msg);
      }
  ...
  async function loadAppointments(status = "pending") {
    try {
      ...
      const url = `/appointments/incoming${
      ...
      <div className="pd-page">
        ...
        <section className="pd-search-note">
          ...
          <div style={{ marginTop: 12 }} className="pd-grid-2">
            ...
            <label style={{ display: "grid", gap: 6 }}>
              <span className="pd-text-muted-sm">
                Filter by Prediction ID (optional)
              </span>
              <input
                className="pd-input"
        ...
        <section className="pd-profile">
          ...
          >
            <h3 className="pd-h3" style={{ marginBottom: 0 }}>
              Patient Profile
            </h3>
            <button
          ...
          {!patientEmail.trim() && (
            <div className="pd-text-muted">
              Enter a patient email and click “Load Profile”.
            </div>
          )}

          {patientEmail.trim() && profileLoading && (
            <div className="pd-loading" style={{ marginTop: 8 }}>
              Loading profile…
            </div>
          )}
          ...
          {patientProfile && !profileLoading && (
            <div className="pd-card" style={{ marginTop: 8 }}>
              ...
              >
                ...
                {(patientProfile.profile.age !== undefined ||
                  patientProfile.profile.Age !== undefined) && (
                  <span className="pd-pill">
                    Age:{" "}
                    {patientProfile.profile.age ?? patientProfile.profile.Age}
                  </span>
                )}
                ...
                {(patientProfile.profile.blood_pressure ||
                  patientProfile.profile.bloodPressure) && (
                  <span className="pd-pill">
                    Blood Pressure:{" "}
                    {patientProfile.profile.blood_pressure ||
                      patientProfile.profile.bloodPressure}
                ...
                {(patientProfile.profile.cholesterol_level ||
                  patientProfile.profile.cholesterolLevel) && (
                  <span className="pd-pill">
                    Cholesterol:{" "}
                    {patientProfile.profile.cholesterol_level ||
                      patientProfile.profile.cholesterolLevel}
                ...
                {(patientProfile.profile.difficulty_breathing !== undefined ||
                  patientProfile.profile.difficultyBreathing !== undefined) && (
                  <span className="pd-pill">
                    Difficulty Breathing:{" "}
                    {patientProfile.profile.difficulty_breathing ||
                    patientProfile.profile.difficultyBreathing
                      ? "Yes"
                      : "No"}
                  </span>
                )}
        ...
        <section className="pd-note-display">
          ...
          {notes.length === 0 && (
            <div className="pd-text-muted">No notes to show.</div>
          )}
          {/* "n" = 1 individual note */}
          {notes.map((n) => (
            <div key={n._id} className="pd-note-box">
              <div className="pd-row">
                ...
                {n.prediction_id && (
                  <span className="pd-pill">
                    pred: {n.prediction_id.slice(0, 8)}…
                  </span>
                )}
        ...
        <section className="pd-appointment">
          ...
          <div className="pd-row" style={{ gap: 8, marginBottom: 12 }}>
            ...
            >
              {apptLoading && statusFilter === "pending"
                ? "Loading…"
                : "Pending"}
            </button>
          ...
          {appointments.map((a) => (
            ...
            >
              ...
              {a.reason && (
                <div
                  className="pd-appt-text"
                  style={{ color: "white", margin: 6 }}
                >
                  Patient's note:{" "}
                  <span style={{ color: "yellow" }}>{a.reason}</span>
                </div>
              )}
              {a.status === "pending" && (
                <div className="pd-row" style={{ marginTop: 10, gap: 8 }}>
                  <button
                    className="pd-decision"
                    onClick={() => updateAppointmentStatus(a._id, "accepted")}
                    disabled={apptLoading}
                  >
                    Accept
                  </button>
